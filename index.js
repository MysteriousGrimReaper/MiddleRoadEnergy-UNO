	...
	while (input_queue[m.channel.id].length > 0) {
			...
			for (c of commands) {
				try {
					...
					await message.channel.send(`An error occurred while executing command: ${c}`)
					console.error(error)
				}
			}
...
for (const file of commandFiles) {
	...
	ref_inputs.push({ names, input: command, execute })
}

const ref_message_listener = async (message) => {
	try {
		let { content } = message;
		content = content.toLowerCase();
		if (
		...
		let is_concat = true
		const input = ref_inputs.find(refi => refi.names.reduce(
			(acc, cv) => {
				if (a[1] == cv) {
					is_concat = false
				}
				return acc || a[0] == `${ref_prefix}${cv}` || a[1] == cv},
			false
		))
		if (input) {
			input.execute(message, ...a.slice(is_concat ? 1 : 2));
		}
	}
	catch (error) {
		console.error(error)
